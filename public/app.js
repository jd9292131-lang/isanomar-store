...
let orderAccess =
  loadLocal(
    "isanomar_order_access",
    {}
  );

function orderQuery() {
  const pairs = orderIds.map(id => ({ id, token: orderAccess[id] })).filter(x => x.token);
  return new URLSearchParams({
    ids: pairs.map(x => x.id).join(','),
    tokens: pairs.map(x => x.token).join(',')
  }).toString();
}
...
async function submitOrder(
  ...
  try {
    ...
    if (response.accessToken) {
      orderAccess[order.id] = response.accessToken;
      saveLocal(
        "isanomar_order_access",
        orderAccess
      );
